...
function App() {
  ...
  const handleFileSelect = async (file) => {
    ...
    try {
      ...
      if (!res?.data?.summary) {
        setMessage("Invalid backend response - missing data summary");
      ...
      setTableData(Array.isArray(res.data.preview) ? res.data.preview : []);
  ...
  const handleAction = async (actionType) => {
    ...
    try {
      ...
      setScore(res?.data?.new_score || score);
      setTableData(Array.isArray(res?.data?.preview) ? res.data.preview : tableData);
      setMessage(res?.data?.message || 'Action completed');
      setAiMessage(res?.data?.message || '');
      setMessageType('success');
    } catch (err) {
      const errorMsg = err.response?.data?.message || err.response?.data || err.message;
      console.error("Action error details:", err.response?.data || err);
